...
- Command bus protocol: full `command` / `response` / `event` envelopes added to `mcp-server/src/bridge/protocol.ts`. `WsBridge` exposes `sendCommand(action, params, opts)` (UUID correlation, configurable timeout, typed errors `BridgeNotConnectedError` / `CommandTimeoutError` / `CommandFailedError`) and `onEvent(handler)` for client-pushed events. New `extension/commands/` module hosts a registry-based dispatcher with a built-in `ping` action; the offscreen document is now an ES module and routes inbound `command` messages through it. New `twin_extension_ping` MCP tool exercises the full server → ext → server round-trip and reports the measured `roundTripMs`.
